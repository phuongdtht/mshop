        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ">
          <div class="panel panel-success">
            ...
            <legend class="text-left">Xác nhận thông tin đơn hàng</legend> 
              ...
              @else 
              <form action="{!!url('/payment')!!}" method="Post" accept-charset="utf-8" style="font-size: 20px;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <div class="form-group">
                  <label for="">
                    <label>Tên khách hàng :</label>
                    <strong class="label label-success">{{ Auth::user()->name }} </strong> <br>
                   <label>Điện thoại liên hệ :</label>
                    <strong class="label label-success"> {{ Auth::user()->phone }}</strong> <br>
                   <label>Địa chỉ nhận hàng :</label>
                    <strong class="label label-success"> {{ Auth::user()->address }}</strong><br>
                    <label>Tổng tiền thanh toán của đơn hàng : </label> <br>                    
                    <strong class="label label-danger"> {!!Cart::subtotal()!!} VNĐ</strong> Tỉ giá : <span style="color:red;">{{ number_format($setting->tigia)}}</span> VNĐ /<span style="color: red;">1$</span> = <label class="label label-success" style="color: red;">{{intval($total/$setting->tigia)}} $</label>
                  </label>
                </div>
                  <br>                
                @if($stt)
                ...
                @else
                  <label style="color:red;"> Để thanh toán Paypal thì chỉ được mua 1 sản phẩm mỗi loại ! vui lòng <a class="btn btn-sm btn-info label-danger" href="{!!url('/gio-hang')!!}" title="update"> cập nhật </a> lại số lượng của bạn, Xin cảm ơn ! </label>
                @endif
              </form>
              @endif
            </div>
